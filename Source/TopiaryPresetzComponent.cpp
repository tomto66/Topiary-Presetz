/////////////////////////////////////////////////////////////////////////////
/*
This file is part of Topiary Presetz, Copyright Tom Tollenaere 2018-2019.

Topiary Presetz is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

Topiary Presetz is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with Topiary Presetz. If not, see <https://www.gnu.org/licenses/>.
*/
/////////////////////////////////////////////////////////////////////////////

#pragma once
#include "TopiaryPresetzComponent.h"

const int TopiaryPresetzComponent::headerHeigth = 75;
const int TopiaryPresetzComponent::width = 1000;
const int TopiaryPresetzComponent::heigth = 500;

TopiaryPresetzComponent::TopiaryPresetzComponent()
{   // don't do too much - model needs to be set first !!
	//getLookAndFeel().setColour(Component:: , Topiary::SLIDER_TRACE_COLOUR);
	//jassert(false);
}

void TopiaryPresetzComponent::setModel(TopiaryPresetzModel* model)
{
	presetzModel = model;
	topiaryPresetzHeaderComponent.setModel(presetzModel); // size set in headerComponent
	addAndMakeVisible(topiaryPresetzHeaderComponent);
	topiaryPresetzTabbedComponent.setModel(presetzModel);
	addAndMakeVisible(topiaryPresetzTabbedComponent); // size set in tabbedComponent 	
}

/////////////////////////////////////////////////////////////////////////

TopiaryPresetzComponent::~TopiaryPresetzComponent()
{	
}



/////////////////////////////////////////////////////////////////////////

void TopiaryPresetzComponent::resized()
{ 
}

/////////////////////////////////////////////////////////////////////////

void TopiaryPresetzComponent::paint(Graphics& g) {
	auto area = getLocalBounds();
	
	g.fillAll(TopiaryColour::background);
	topiaryPresetzTabbedComponent.setBounds(area.removeFromBottom(TopiaryPresetzComponent::heigth-TopiaryPresetzComponent::headerHeigth-10));
	topiaryPresetzHeaderComponent.setBounds(area.removeFromTop(TopiaryPresetzComponent::headerHeigth));
	
}









